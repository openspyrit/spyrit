...
def test_core_time():
    ...
    assert_test(warped_img.shape, torch.Size([10, 1, nx, ny]), "Wrong forward greyscale size")
    
    # forward color (3D)
    img = torch.randn(3, nx, ny, dtype=torch.float)
    warped_img = def_field(img, 0, n_frames)
    print("forward color:", warped_img.shape)
    assert_test(warped_img.shape, torch.Size([10, 3, nx, ny]), "Wrong forward color size")
    ...
    assert_elementwise_equal(warped_img, torch.tensor([[[3, 1], [4, 2]]], dtype=torch.float), "Wrong forward rotating clockwise")
    ...
    assert_elementwise_equal(warped_img, torch.tensor([[[[2, 4], [1, 3]]]], dtype=torch.float), "Wrong forward single image")
    ...
    assert_elementwise_equal(warped_img, torch.tensor([
        [[[1, 2], [3, 4]]], 
        [[[2, 4], [1, 3]]], 
        [[[4, 3], [2, 1]]], 
        [[[3, 1], [4, 2]]]
