...
class DeformationField(nn.Module):
    ...
    **Example 2:** Rotating a 2x2 B&W image by 90 degrees clockwise, using one frame
    ...
    def _warp(self, img_collection, inverse_grid_frames, mode):
        ...
        img_collection = img_collection.reshape(
        ...
        ).to(img_collection.dtype).reshape(n_frames, c, n_pixels)
    ...
    def _warn_field(self):
        ...
        if self.field.dtype == torch.float32:
            ...
            if self.__class__ == AffineDeformationField:
                msg = "Consider using float64 when defining the output " \
        ...
        if self.warn_range and (self.field.abs() > 2).any():
            msg = "The deformation field goes beyond the range [-2;2], " \
