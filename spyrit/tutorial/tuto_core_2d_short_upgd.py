...
from spyrit.core.recon import PseudoInverse, PinvNet, DCNet, UPGD
...
if (load_unet is True):
    ...
    z_plot = z_upgd.view(-1,H,H).detach().cpu().numpy()
    imagesc(z_plot[0,:,:],'UPGD UNet reconstruction', show=False)
    plt.show()
