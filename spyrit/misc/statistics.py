...
def stat_stl10(stat_root = Path('./stats/'), 
                data_root = Path('./data/'),
                img_size:int = 64, 
                batch_size:int = 1024,
                get_size:str='resize',
    ...
    Args:
        ...
        :attr:`data_root`: path to image database.  :attr:`data_root` is expected 
        to contain an `/stl10_binary/` subfolder with the  `test*.bin`, `train*.bin` 
        ...
        :attr:`get_size`: specifies how images of size :attr:`img_size` are 
        obtained (see :mod:`~spyrit.misc.statistics.data_loaders_stl10`)
    
    Example:
        data_root =  Path('../datasets/')
        stat_root =  Path('../stat/stl10')
    
        from spyrit.misc.statistics import stat_stl10
        stat_stl10(stat_root = stat_root, data_root = data_root)
...
def data_loaders_stl10(data_root, 
                       img_size:int = 64, 
                       batch_size:int = 512, 
                       seed = None, 
                       shuffle = False, 
